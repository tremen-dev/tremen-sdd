...
2. Copia desde `${CLAUDE_PLUGIN_ROOT}/core/templates/` a la raíz del proyecto,
...
3. Crea `docs/epicas/` y `docs/adr/` vacíos y ejecuta
   `node "${CLAUDE_PLUGIN_ROOT}/core/scripts/tablero.mjs"` para el primer tablero.
4. Ofrece generar roles de dominio desde
   `${CLAUDE_PLUGIN_ROOT}/core/templates/rol-dominio.md`: por cada uno, pide nombre
...
3. NO MUEVAS NADA sin aprobación explícita del humano. Con aprobación, migra
   por lotes pequeños, ejecutando `node "${CLAUDE_PLUGIN_ROOT}/core/scripts/valida.mjs"`
