...
- **Fase 2**: i18n gl/en (`roles/gl/`, `roles/en/`, plantillas traducidas, selector en init); generador de roles de dominio asistido; evals completos; compatibilidad multi-copiloto (AGENTS.md/GEMINI.md punteros a `.ai-context/`, patrón lidr-ai-specs); empaquetado "profesional" (`npx tremen-sdd init`, skills.sh, npm) consumiendo el mismo `templates/`.
